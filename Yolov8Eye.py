...
# Load YOLOv8 model
model = YOLO(os.path.join(BASE_DIR, 'yolov8/yolov8n.pt'))  # Use the appropriate YOLOv8 model file trained for face detection
...
def login_user():
    ...
    if username.lower() in current_users and current_users[username.lower()][1] == user_id:
        session['username'] = current_users[username.lower()][0]  # Store original username case
        return redirect(url_for('home'))
    else:
...
def generate_frames():
    ...
    if not cap.isOpened():
        print("Error: Could not open webcam. On Mac, check System Settings > Privacy & Security > Camera to grant camera permissions.")
